...
2026-09-11までに確認・共有された状態を示す。今回の文書更新では学習・評価・成果物checkを再実行していない。
...
| 主ニューラル条件 | A0・M11のfold 1–2、M00・M10・M01のfold 1の各repeat 1–3で学習・clustering・評価・checkが完了 |
| 主実験の完了数 | NN学習・clustering・評価は各21/75。B0・B1を含むclustering・評価は各51/105組合せ |
| OOF sanity | B0・B1のPNG 3枚・CSV 3つを生成済み。[表示仕様](docs/design/oof_sanity_visualization.md) |
| 実行中 | なし（最終確認時点） |
| 次のrun | A0・fold 3・repeat 1–3。同repeatの学習 → clean test map → clustering checkを順次実行し、3反復をまとめて全test評価 → evaluation checkまで完了する |
...
- [ ] A0のfold 3–5・repeat 1–3を完了する（6/15 runs完了。fold 1–2は完了）。
