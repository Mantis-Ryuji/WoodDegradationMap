...
本構成との近さは、encoder・decoderの構成、再構成の学習課題、得た表現の用途に分けて考える。
Georgievらは非線形encoderと線形decoder、Renらはmask学習とクラスタリング、Jensenらは
noise・軸方向変動からの復元、Jiらは画素表現から空間マップと新規データ適用までの流れに対応する。
一つの先行研究を本研究全体の検証とせず、以下では共通点に続けて、制約・目的・評価の違いを示す。
...
調査した前例のうち、Georgievらは**スペクトルからglobalな潜在を求め、非線形encoderと線形decoderで
再構成するという構成上の比較対象**として特に近い。Ramanスペクトルの非線形encoderに、
biasなしの線形decoderを組み合わせる。
...
本研究が問うのは、化学成分への分解を課すことなく、mask再構成から状態に関連するまとまりが
生じるかである。したがって、Georgievらは線形decoderの構成と、その解釈に必要な制約を説明するために
引用する。MAEという学習課題と、得た特徴によるクラスタリングは、次節のRenらを比較対象とする。
...
以上の前例は構成や用途の選択を説明する材料となるが、decoderの単純さが固定表現の品質を
保証するとは言えない。本構成ではdecoderが非線形な処理を担えないため、encoder側に
「線形に復元できる座標」を作る役割がある。その座標が分類・クラスタリングにも有用かどうかは、
構造の説明から分けて検証する必要がある。
...
参照目的ごとに書誌をまとめ、本文・abstract・技術資料のどこまで確認したかを各項目に残す。
