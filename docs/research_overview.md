...
本書は「何を問うか → どの手法で取り組むか → どの証拠で答えるか」の順に研究の論旨を示し、
その論旨に基づく日本語・英語の説明文をまとめる。
...
[ChemoMAEの位置づけ](chemomae_positioning.md)で検討する。先行研究の役割は、構成全体の前例を一つに求めず、
利用目的・学習課題・モデル構成・解析手順の各側面で整理する。

| 説明する側面 | 引用候補 |
| --- | --- |
| 非線形な低次元表現を得る利用目的 | 非線形PCA：[Kramer, 1991](https://doi.org/10.1002/aic.690370209) |
| Denoisingを通じた表現学習 | Denoising AE：[Vincent et al., 2010](https://jmlr.org/papers/volume11/vincent10a/vincent10a.pdf) |
| 非線形encoderと線形decoderの組合せ | Raman unmixing AE：[Georgiev et al., 2024](https://doi.org/10.1073/pnas.2407439121) |
| Mask学習とクラスタリング | Raman SMAE：[Ren et al., 2025](https://arxiv.org/html/2504.16130v1) |
| 画素スペクトルの教師なし圧縮・マップ化と、固定表現・中心の新規データへの適用 | LeafVAE：[Ji et al., 2026](https://doi.org/10.1016/j.compag.2026.111971) |

各研究との相違点と出典を確認した範囲は[関連研究](related_work.md)にまとめる。
...
修士論文では、問いと証拠を章に対応づける[構成案](../thesis/outline.md)と、
式・図表・参照資料を対応づける[執筆計画](../thesis/writing_plan.md)に展開する。
本書は研究全体の説明に用い、原稿の掲載位置や執筆状況は`thesis/`側で管理する。
