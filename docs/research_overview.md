...
| 表現差を説明する補助診断 | クラスタ平均、実画素間・同画素の摂動前後、SVD・残差 | どの入力差が潜在で強調・抑制されたか。平均で隠れる変動は何か |
| 化学的な意味を検討する | NIRの代表・差スペクトル、試料情報、位置対応FT-IRの計画 | 分割された領域の差を化学状態とどう対応づけられるか。劣化以外の説明は何か |

補助診断はCV開始後に合意した探索的計画である。MAE4条件を軸にクラスタ・画素の両方を扱い、A1等は追加しない。
対象・抽出・数値規約は[補助診断計画](design/representation_geometry_diagnostics.md)のOpen事項として管理する。
