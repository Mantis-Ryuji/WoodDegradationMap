...
クラスタ数は、データから推定すべき真の状態数ではなく、連続潜在空間を異なる粒度で観察するための
resolution parameterとして扱う。全試料を用いたelbowなどによる選択はheld-out試料の分布を
CV設計へ持ち込む。一方、train foldだけで選択すると直接のtest leakageは避けられるが、fold間・
条件間でKが変化し、同じ試料・K・反復に基づくpaired比較が困難になる。
...
主表および全体可視化に使用する代表表示値を、共通集合の中央の値である8に固定する。これは、
粗い主要構造と細かな局所変動の中間的な表示解像度を与える、運用上の共通基準である。
...
対象はいずれも木材であり、SNV後のNIRスペクトルでは主要な吸収構造を共有する。樹種差は存在し得るが、
樹種ごとに互いに独立した離散的スペクトル状態を形成するとは事前に仮定しない。このため、樹種はこの
組合せの水準数には加えず、年輪組織、劣化状態および環境由来変動のスペクトル上の位置や現れ方を
修飾し得る背景因子として扱う。ただし、樹種差が実際のクラスタ形成へ寄与しないことを仮定するものではない。

この$2\times2\times2$は$K_0$へ現象上の粗い解釈を与える補助的なヒューリスティックであり、
各クラスタへこれらの要因を事前に割り当てるものではない。各要因は実際には連続的で相互作用し、
クラスタと1対1に対応する保証もない。$K_0=8$の第一の根拠は、事前固定した共通K集合の中央に位置する
中間的な表示解像度であることに置く。

したがって、$K_0$は表示上の代表値であり、データから推定した真のクラスタ数や最適K、8つの
離散的な化学状態を意味しない。条件ごと、foldごと、評価指標ごとに都合のよいKを選ばない。
...
実装・preflightの確認結果は[../verification_history.md](../verification_history.md)、本番の
実行手順は[../experiment_runbook.md](../experiment_runbook.md)に記録する。本番開始時は専用
