...
[実験プロトコル](experiment_protocol.md)では、M11対B0・B1・M00の主要比較と、
M00・M10・M01・M11の2×2比較・交互作用を、共通の試料・K・反復で評価する。
TGN・shiftの単独・追加・併用によってマップの性質がどう変わるかは、この比較と[既定指標](evaluation_metrics.md)で検討できる。
...
B.5.2・B.5.5の恒等式は、再構成lossと潜在のcosine幾何が異なる量を測ることを説明する。
恒等式の数値照合だけではTGN・shiftの有効性を実証せず、学習による改善機構の証明にもならない。
一式の診断には共通分割・画素対・重み・数値規約・保存物の新規設計と実装が必要であり、
現行の問いを評価するための必須作業にはしない。
...
**以下の第2〜9節は、再検討時に参照する2026-09-12の旧案である。記載した手順・Open事項は、現在の残作業や実行予定を意味しない。**

旧案の問いは「入力のどの差が潜在で強調・抑制され、その対応がMAE群のcorruption条件でどう変わるか」であった。
旧案で設けた範囲・制約を以下に残す。
...
[実験プロトコル](experiment_protocol.md)、[評価・集計規約](evaluation_metrics.md)、[全体fitと解釈](visualization_and_interpretation.md)を継承した旧案である。CV開始後に立てた探索的な補助診断であり、事前の主要仮説・指標として遡及記載しない。
...
## 7. 旧案の実施順序・成果物・論文への反映

以下は旧案の対応表であり、この一式を実施する予定はない。再採用する場合も、具体的な問いに必要な部分だけを選ぶ。
...
上記は未採用候補の検討の起点であり、現在のOpen作業一覧には含めない。採用範囲を別途決めた場合の実行はリポジトリの実行規約に従う。
