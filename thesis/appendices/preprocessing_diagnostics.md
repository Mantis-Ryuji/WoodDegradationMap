...
[第3.2節](../chapters/3_analysis_methods/preprocessing.md)を補足し、A.1〜A.4で前処理の定義、A.5で既存診断図の集計対象と解釈を示す。
...
波長選択の診断図と反射率の大きさを表すmapを示す。それぞれの集計対象を、最終学習入力・画素除外率と区別する。
...
**図A.1 Reference由来SNR proxyによる終端波長範囲の選択。** 横軸は元の測定波長（nm）、縦軸は対数尺度のreference由来SNR proxy $\gamma_b$ である。黒線でproxy、青破線で固定閾値10を示す。保持範囲を淡緑色、除外範囲を淡橙色、両者の境界2308.72 nmを橙色の縦点線で示す。波長範囲を決めるreferenceの判定量だけを単一panelで表示する。

#### A.5.1.1 表示値と対象波長

横軸は除外範囲も含む元の256測定bandsであり、最終入力の補間gridではない。背景色はbandの保持・除外を表す。

黒線は式(A.3)の $\gamma_b$、青破線は閾値10を表す。正かつ有限のproxyを対数軸へ描画する。非有限値も除外判定の対象なので、非表示の値を保持可能とはみなさない。

#### A.5.1.2 この図から読み取る内容

長波長端でproxyが低下し、閾値10以下となる連続区間が終端まで続く。固定閾値と終端連続区間の規約により、この34 bandsを除外する。Proxyはreferenceの列方向のばらつきに対する比であり、試料スペクトルの分布や時間方向の測定ノイズを直接表すものではない。

試料スペクトル、最終マップまたはCV評価を用いて閾値や境界を探索したものではない。境界2308.72 nmは保持・除外bandの間を示す描画上の位置であり、補間の上限は最後の保持測定波長2305.59 nmである。
...
- 図A.1の描画：[描画処理](../../src/wood_degradation_map/preprocessing/visualization.py)。表示値は元測定bandのreference品質表に保存したSNR proxyであり、試料のband分布や補間後のSNVを表示する図ではない。
- 図A.1の既存出力：[cutoff_decision.png](../../outputs/preprocessing/production_v1/cutoff_decision.png)。

**残る整備。** 試料別の有効・除外画素数、マスク前後の例、SNVの数値誤差、TGN・shiftの強度確認図は、元資料の照合後に追加する候補とする。
