...
**第1段：Reference SNR proxy。** 黒線は式(A.3)の $\gamma_b$、青破線は閾値10を表す。正かつ有限のproxyを対数軸へ描画する。非有限値も除外判定の対象なので、非表示の値を保持可能とはみなさない。
...
縦軸は0〜1の割合である。最大値を与える試料はband・項目ごとに異なり、三曲線の分母も共通でないため、合計を除外率とはみなせない。各bandの試料別最大割合と、複数bandをまとめて判定する最終の画素除外率は別の量である。
...
第4段と学習入力 $\boldsymbol{x}_p$（$p$ は有効画素）のSNVは、平均・標準偏差を計算する波長点と対象画素が異なる。最終入力は波長除外、222点から256点への補間、品質判定を経るため、第4段をその分布として用いない。
