...
を満たす。これは16次元のアフィン部分空間内の楕円体表面の制約を表す。潜在 $\boldsymbol{z}$ 自体は単位球面上にあり、楕円体が現れるのはdecoder後の復元空間である。固定したdecoderが全単位潜在から作れる集合は $\boldsymbol{b}_{\mathrm{dec}}+W_{\mathrm{dec}}\mathbb{S}^{15}$ であり、encoderはその集合上のどの復元値を入力に対応づけるかを非線形に推定する。重みが列full rankでない場合は像が退化し、この15次元の楕円体表面としての説明はそのまま適用できない。実際のencoder出力が単位球面全体を覆うとは限らないため、学習した観測の復元値はこの集合の一部にしか現れない場合がある。また、球面の次元が15であることと、そのアフィン包の次元が最大16であることは矛盾しない。
...
第3.5節の単位クラスタリング入力を $\boldsymbol{\xi}_p$、第 $k$ クラスタの単位中心を $\boldsymbol{c}_k$ とする。$p$ は画素添字、$k$ はクラスタ添字である。両者に対し、
...
入力チャネル数を $C=256$、SNVスペクトルのnormを $\rho=\sqrt{C-1}$ とする。B.1の制約集合 $\mathcal{S}_{\mathrm{SNV}}$ では、全帯域の平均とnormがすでに固定されている。このため、同じnormのSNVスペクトルを区別する情報は、平均ゼロの部分空間内での方向にある。ここでいう方向は全チャネルの相対的な値の組合せを含み、スペクトル形状を一つの角度だけで表せるという意味ではない。
...
執筆メモ：B.1〜B.5は第3章の定義からの導出であり、実験結果ではない。使用中のChemoMAE v0.2.2のaugmentation・モデル・masked MSE、および[呼出し側](../../src/wood_degradation_map/experiments/neural.py)を読み取りで照合した。関連する既存の説明は[ChemoMAEの位置づけ](../../docs/chemomae_positioning.md)。標準手法の原典と本文の引用は最終稿で整理する。

執筆補助資料：2026-09-12に[会話「内容の意味解説」](https://chatgpt.com/c/6aa41285-9758-83ee-9d0f-8ca3daa23ff6)を参照し、B.5の説明とB.5.4〜B.5.7を補足した。会話は論点の整理に用い、数学的な根拠は上記の式の導出と[現行プロトコル](../../docs/design/experiment_protocol.md)に対応する定義による。会話中のSNVのnormは本研究の標本標準偏差に合わせて $\sqrt{C-1}$ とし、PCAのfit平均中心化、decoderのbias、全チャネルMSEとmasked MSEの違いを反映した。球面化・化学的な分離・性能改善を実験結果として採用していない。新しい制約や評価実験は追加しておらず、学習済み重みの数値検証も実施していない。
