...
各画素の表現を単位化し、共通のcosineクラスタリングによるラベルを元画像へ配置する。
...
PCA score $\boldsymbol{\psi}_p$ のnormは、fit平均からのスペクトル形状のずれが、選んだ主成分部分空間にどれだけ含まれるかを表す。SNV前の反射率の絶対的な大きさではない。後段の単位化はこのscoreの大きさをクラスタリングに用いない処理である。
...
- **参照資料・照合先：** 現行仕様は[実験プロトコル](../../../docs/design/experiment_protocol.md)、実装は[baseline](../../../src/wood_degradation_map/experiments/baselines.py)、[クラスタリング](../../../src/wood_degradation_map/experiments/clustering.py)。
- **残る整備：** PCAおよび球面クラスタリングの原典の引用を追加する。vMFは数値仕様が未確定のため、本節では主手法のCosine-KMeansを記述する。
