...
## 10. 付録B.5以降：数理的補足としての範囲

付録B.5は、入力差・潜在差・残差の対応、クラスタの分離と内部の広がり、同一画素への摂動応答、masked lossが測る変動の数理的関係を補足する。算術平均を使う関係式は本文用の中央値による代表線と区別する。

対応する補助診断一式は、2026-09-13の[必要性の見直し](../docs/design/representation_geometry_diagnostics.md)により現行の実施計画から外した。比較対象・抽出・数値規約の確定や診断実装、専用の結果図表は残作業に含めない。

本文から必要な数理的関係を参照し、式の成立をTGN・shiftの効果や改善機構の実証とは扱わない。効果の比較は既定の2×2比較・LLA・LFRと診断指標に基づき、化学的な意味は観測スペクトル等から検討する。具体的な未解決の問いに対して追加解析を採用した場合だけ、その探索的位置づけと実施範囲を記述する。
